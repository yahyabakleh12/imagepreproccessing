...
"%LPD_API_PYTHON%" -c "import fastapi, uvicorn, multipart, cv2, ultralytics, openvino, torch, transformers, bm3d, skimage, fire" >nul 2>&1
if errorlevel 1 (
    echo ERROR: Required Python packages are missing.
    echo.
    echo Run this command from this folder:
    echo "%LPD_API_PYTHON%" -m pip install -r requirements.txt
    echo.
    pause
    exit /b 1
)

start "" powershell.exe -NoProfile -WindowStyle Hidden -Command "Start-Sleep -Seconds 8; Start-Process 'http://localhost:8002'"
"%LPD_API_PYTHON%" -m uvicorn lpd_api:app --host 0.0.0.0 --port 8002
